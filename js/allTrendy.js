...
const trendyRow = document.getElementById("trendy_cards_row");
...
function renderGames(filter) {
  ...
  games.forEach(game => {
    if (
      filter === "Arcade" && game.Category==='Arcade' && parseInt(game.Trendy)>=95||
      (filter === "Action" && game.Category==='Action' && parseInt(game.Trendy)>=95) ||
      (filter === "Puzzle" && game.Category==='Puzzle' && parseInt(game.Trendy)>=95) ||
      (filter === "Simulation" && game.Category==='Simulation' && parseInt(game.Trendy)>=95) 
    ) {
...
categorySelect.onchange = e => renderGames(e.target.value);